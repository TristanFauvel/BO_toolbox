...
for i = 1:model.nsamples+1
    u = [ones(1,i-1), zeros(1, model.nsamples-i+1)] ;
...
%     if nargout>1
        [mu_c,  mu_y, sigma2_y, Sigma2_y, dmucdx, dmuydx, dsigma2ydx, dSigma2y_dx] =  prediction_bin(theta, xtrain_norm, ctrain, xduels, model, post);
        
          
        dSigma2y = zeros(nd,nd,nduels);
...
%     else
%         [mu_c,  mu_y, sigma2_y, Sigma2_y] =  prediction_bin(theta, xtrain_norm, ctrain, xduels, model, post);
%     end
end
...
%     [~, res(:,i)] = pref_MI(theta, xtrain_norm, ctrain, x_best_norm, x(:,i), model, post);
% end
% figure();
