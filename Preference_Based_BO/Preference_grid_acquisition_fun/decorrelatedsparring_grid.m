...
idx = unique(idx);
x0 = x(:,1);
[~,  ~, sigma2_y, Sigma2_y]  =  model.prediction(theta, xtrain, ctrain, [x(:,idx);x0*ones(1,numel(idx))], kernelfun, kernelname, modeltype, post, regularization); %kernelfun(theta, [x(:,idx);x0*ones(1,numel(idx))], [x(:,idx);x0*ones(1,numel(idx))]);
