...
for i = 1:size(xduels,2)
    [mu_c_1, ~, ~] = prediction_bin_preference(theta, [xtrain, xduels(:,i)], [ctrain, 1], xduels, kernelfun,kernelname, 'modeltype', modeltype, 'post', post);
    [mu_c_0, ~, ~] = prediction_bin_preference(theta, [xtrain, xduels(:,i)], [ctrain, 0], xduels, kernelfun, kernelname,'modeltype', modeltype, 'post', post);
