...
for i =1:size(x,2)
    %case y = 0
    [~, ~, sigma2_y0] =  prediction_bin(theta, [xtrain_norm, x(:,i)], c0, xnorm, kernelfun, modeltype, [], regularization);
    %case y = 1
    [~,~, sigma2_y1] =  prediction_bin(theta, [xtrain_norm, x(:,i)], c1, xnorm, kernelfun, modeltype, [], regularization);
