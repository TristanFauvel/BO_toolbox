...
[mu_c,  mu_y, sigma2_y] =  model.prediction(theta, xtrain_norm, ctrain, xnorm, post);
...
for i =1:size(x,2)
    %case y = 0
    [~, ~, sigma2_y0] =  model.prediction(theta, [xtrain_norm, x(:,i)], c0, xnorm, post);
    %case y = 1
    [~,~, sigma2_y1] =  model.prediction(theta, [xtrain_norm, x(:,i)], c1, xnorm, post);
