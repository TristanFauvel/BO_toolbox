...
for i =1:maxiter
    ...
    [mu_c, mu_y, sigma2_y, ~, ~, ~, ~, ~, var_muc] = prediction_bin(theta, xtrain_norm(:,1:i), ctrain(:,1:i), x, model, post);
...
%     score_c(i) = sqrt(mse(mu_c(:),link(y(:))));
    ...
    if i > ninit
        init_guess = theta;
        theta = multistart_minConf(@(hyp)negloglike_bin(hyp, xtrain_norm(:,1:i), ctrain(:,1:i), model, 'post', post), theta_lb, theta_ub,10, init_guess, options_theta); 
    end
    if i> nopt              
