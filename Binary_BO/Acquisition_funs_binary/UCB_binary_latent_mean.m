function [new_x,new_x_norm] = UCB_binary_latent_mean(theta, xtrain_norm, ctrain, model, post, approximation, varargin)
...
ncandidates= 5;
init_guess = [];
new_x_norm = multistart_minConf(@(x)ucb(theta, xtrain_norm, x, ctrain, model, post, task), model.lb_norm, model.ub_norm, ncandidates, init_guess, options);
...
function [ucb_val, ducb_dx]= ucb(theta, xtrain_norm, x, ctrain, model, post, task)

% if strcmp(task, 'max')
    [mu_y, sigma_c, dmuy_dx, dsigma_c_dx] =  gpbin(theta, xtrain_norm, ctrain, x, model, post);
% elseif strcmp(task, 'average')
%     mu_y = integral(@(s) fmu_c(theta, xtrain_norm, ctrain, [s;x*ones(1,size(s,2))], model, post), zeros(1,model.ns), ones(1,model.ns), 'ArrayValued', true);
...
%     dmuy_dx = integral(@(s) fdmuc_dx(theta, xtrain_norm, ctrain, [s;x*ones(1,size(s,2))], model, post), zeros(1,model.ns), ones(1,model.ns), 'ArrayValued', true);
...
% end
e = norminv(0.975);
ucb_val = mu_y + e*sigma_c;
ucb_val = -ucb_val;
ducb_dx = -(dmuy_dx(:) + e*dsigma_c_dx(:));
end
...
function [mu_y, sigma_c, dmuy_dx, dsigma_c_dx]  = gpbin(theta, xtrain_norm, ctrain, x, model, post)
[mu_c,  mu_y, sigma2_y, Sigma2_y, dmuc_dx, dmuy_dx, dsigma2y_dx, dSigma2y_dx, var_muc, dvar_muc_dx] =  prediction_bin(theta, xtrain_norm, ctrain, x, model, post);
sigma_c = sqrt(var_muc);
dsigma_c_dx = dvar_muc_dx./(2*sigma_c);
% dsigma_c_dx = dsigma_c_dx((model.ns+1):end);
