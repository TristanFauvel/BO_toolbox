...
[mu_c,  mu_f, sigma2_f] = prediction_bin(theta, xtrain(:,1:ntr), ctrain(1:ntr), x2d, kernelfun, 'modeltype', modeltype);

[~,  mu_g, sigma2_g, Sigma2_g] = prediction_bin(theta, xtrain(:,1:ntr), ctrain(1:ntr), [x; x0*ones(1,n^d)], kernelfun, 'modeltype', modeltype);
...
nexttile
i=i+1;
plot(x, sample_g(x),'k','linewidth',linewidth); hold on;
[sample_gmax, id] = max(sample_g(x));
xmax = x(id);
